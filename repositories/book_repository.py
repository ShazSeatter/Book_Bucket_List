...
def save(book):
    sql = "INSERT INTO books (title, author_id, completed, review) VALUES (%s, %s, %s, %s) RETURNING *"
    values = [book.title, book.author.id, book.completed, book.review]
...
def select(id):
    ...
    if results:
        ...
        book = Book(result['title'], author, result['completed'], result['review'], result['id'])
...
# review
def update(book):
    sql = "UPDATE books SET (title, author_id, completed, review) = (%s, %s, %s) WHERE id = %s"
